<template>
  ...
  <div class="surface-section">
      ...
      <div class="flex align-items-start flex-column md:flex-row">
        <div>
          <div class="flex align-items-center flex-wrap text-sm">
            ...
            <SharedFiles :types="['coach_report','coachee_report']" :coachee_id="coacheeId" />
          </div>
        </div>
      </div>
      <!-- <div class="mt-3">
        ...
        <Button
            class="p-button-icon-only p-button-text p-button-plain p-button-rounded mr-1"
            icon="pi pi-cog"
            v-tooltip.top="{ value: 'Change Password', class: 'text-sm' }"
        />
    ...
    <div
      class="
        surface-card
        border-1 border-gray-200 border-round
        flex
        px-3
        my-4
        flex-column
    ...
    <coacheeTimeline
      :coacheeId="coacheeId"
      :projectId="coacheeData.projectCodeId"
    />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getAvatar(id) {
      ...
      if (id) {
        img =  axios.defaults.baseURL +`/assets/${id}?key=profile-display-image`;
      }
      return img;
    },
  },
